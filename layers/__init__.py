...
def make_loss_with_center(cfg, num_classes):    # modified by gu
    ...
    ls1_loss = CrossEntropyLabelSmooth(num_classes=num_classes)
    ls2_loss = CrossEntropyLabelSmooth(num_classes=num_classes)
    ls3_loss = CrossEntropyLabelSmooth(num_classes=num_classes)

    def loss_func(output_2, output_3, score, feat, target):
        if cfg.MODEL.METRIC_LOSS_TYPE == 'center':
            print("MODEL METRIC_LOSS_TYPE CENTER")
            if cfg.MODEL.IF_LABELSMOOTH == 'on':
                return ls2_loss(output_2, target) + ls3_loss(output_3, target) + xent(score, target) + \
